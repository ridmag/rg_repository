...
DROP TABLE IF EXISTS referenceitems cascade;
...
DROP TABLE IF EXISTS GroupedStaffWeekDaysXref;
DROP TABLE IF EXISTS GroupedStaff;
...
DROP TABLE IF EXISTS StaffNotAvailableDayXRef;
...
CREATE INDEX filedata_blobId_index ON filedata USING btree (blobId);

alter table filedata 
	ADD CONSTRAINT FK_filedata_blobId FOREIGN KEY (blobId) REFERENCES blobdata(id) MATCH FULL;
ALTER TABLE ONLY staffmembers
...
CREATE TABLE programevents (
  ...
  lunchincluded boolean,
  chargeAmount double precision,
  totalMoneyCollected double precision,
  totalEFTCollected double precision,
...
CREATE TABLE ProgramEventCoordinatorXRef(
	...
	status varchar(10),
	primary key(id, programEventId, coordinatorId));
...
CREATE TABLE invoices(
	...
	otherCharge double precision NULL,
...
CREATE TABLE studentevents (
  ...
  studentFundingSourceId bigint not null,
...
CREATE INDEX studentevents_studentFundingSourceId_index ON studentevents USING btree (studentFundingSourceId);
...
CREATE TABLE HoursUtilizedReportItem(
  ...
  programMinsUsed double precision,
  transportMinsUsed double precision,
  balanceMins double precision,
  remarks varchar(255),
  PRIMARY KEY (id)); 
