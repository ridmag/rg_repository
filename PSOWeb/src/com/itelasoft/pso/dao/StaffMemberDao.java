...
public class StaffMemberDao extends GenericDao<StaffMember, Long> implements
	...
	public List<StaffMember> listAvailableByNameNGroup(Long groupId,
		...
		Query query = session
				.createQuery("select staff from StaffMember as staff, StudentGroup as g "
						...
						+ "and staff not in elements(g.staffMembers) and staff.status = 'Current' order by staff.contact.firstName,staff.contact.lastName asc");
	...
	public List<StaffMember> listAvailableByStaffIdNGroup(Long groupId, String staffId) {
		//:TODO: Fix this to filter by group
		Session session = getCurrentSession();
		Query query = session
				.createQuery("select staff from StaffMember as staff where staff.staffId like :staffId "
						+ "and staff.status = 'Current' order by staff.contact.firstName,staff.contact.lastName asc");
		/*Query query = session
		...
		Criteria criteria = session.createCriteria(StaffMember.class);
		if(!staffId.isEmpty()){
			criteria.add(Restrictions.eq("staffId", staffId).ignoreCase());
		}
		//criteria.add(Restrictions.eq("id", groupId));
		StaffMember member = (StaffMember) criteria.uniqueResult();*/
